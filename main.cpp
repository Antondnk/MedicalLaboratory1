...
int main() {
    ...
    std::cout << "=== ДЕМОНСТРАЦИЯ РАБОТЫ МЕДИЦИНСКОЙ ЛАБОРАТОРИИ ===\n\n";

    // 1. Создание объектов (виды анализов)
    std::cout << "--- 1. Доступные виды анализов ---\n";
    ...
    // 2. Изменение характеристик объекта (по заданию)
    std::cout << "\n--- 2. Изменение стоимости ---\n";
    std::cout << "Изменяем цену на глюкозу с 300 на 350 руб...\n";
    sugarTest.setCost(350.0);
    sugarTest.printInfo(); // Проверяем, что цена изменилась
    ...
    std::cout << "Добавляем результаты пациенту...\n";
    ...
    patient1.addTestResult(duplicateRes); // Тут должна выскочить наша красная ошибка!
